...
_update_track( xmmsv_t* value, void* user_data )
{
	...
	/* FIXME: can't be a local variable ?????????*/
	track_properties = E_MUSIC_NEW(TrackProperties, 1);
...
_on_playlist_content_received( xmmsv_t* value, Evas_Object *playlist_show )
{
	...
	for ( i = 0; i < pl_size; i++ ) 
	{
		...
		/*  FIXME: we have to do this. the resoult storn in res???  
		    The genlist will use the variable more than one time, so we must not free them   */
		//		xmmsc_result_unref( res );
	}
...
_on_cover_retrieve( xmmsv_t *value, void *user_data )
{
	...
	if( xmmsv_get_bin( value, &bc, &bl ) )
	{
		...
		/* we have to give the size and fill first, or we can't catch the o_data
		   the size should be big enough ,but how big it should be?? we only have the bl */
		evas_object_image_size_set(o, 64, 64);
		evas_object_image_fill_set(o, 0, 0, 180, 180);
		...
		memcpy ( o_data, bc, 64*64*4);
		evas_object_image_data_set(o, o_data);
		evas_object_image_data_update_add(o, 0, 0, 64, 64);
		elm_layout_content_set(sd->layout, "cover.swallow", o);
