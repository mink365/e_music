...
void 
emusic_collections_creat ( xmmsc_connection_t *conn, char *pattern )
{
...
void 
emusic_collections_add_to_playlist( xmmsc_connection_t *conn, char *playlist)
{
...
void emusic_mlib_browse_update_artists()
{
	...
	const char    *properties[] = { "artist", "album", "picture_front", NULL };
	const char    *group_by[]   = { "album", NULL };


	universe = xmmsc_coll_universe();

	
	res = xmmsc_coll_query_infos(em->conn, universe, create_native_value( group_by ),
